...
export class AssetManager implements IAssetManager {
    ...
    public async downloadAsset(url: string | File): Promise<boolean> {
        return await this.downloadAssets([url]);
    }

    public async downloadAssets(urls: (string | File)[]): Promise<boolean> {
        if (!urls || !urls.length) return Promise.resolve(true);

        try {
            for (const urlOrFile of urls) {
                let response;
                ...
                if (typeof urlOrFile === 'string') {
                    response = await fetch(urlOrFile);
                ...
                switch (contentType) {
                    ...
                    case 'image/gif': {
                        ...
                        const createAsset = async () => {
                            ...
                            this.setTexture(typeof urlOrFile === 'string' ? urlOrFile : urlOrFile.name, texture);
                        };
